...
import com.mengcraft.jopt.internal.Rows;
...
public class BuiltinHelpFormatter implements HelpFormatter {
   ...
   public String format(Map options) {
      Comparator comparator = new Comparator<OptionDescriptor>() {
   ...
   private boolean hasRequiredOption(Collection options) {
      ...
      while(var3.hasNext()) {
         OptionDescriptor each = (OptionDescriptor)var3.next();
         if(each.isRequired()) {
            return true;
         }
      }
   ...
   private String createOptionDisplay(OptionDescriptor descriptor) {
      ...
      Iterator i = descriptor.options().iterator();
