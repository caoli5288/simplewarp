...
class Columns {
   ...
   List fit(Row row) {
      List options = this.piecesOf(row.option, this.optionWidth);
      List descriptions = this.piecesOf(row.description, this.descriptionWidth);
   ...
   private List piecesOf(String raw, int width) {
      ...
      String[] var7;
      int var6 = (var7 = raw.trim().split(Strings.LINE_SEPARATOR)).length;

      for(int var5 = 0; var5 < var6; ++var5) {
         String each = var7[var5];
